...
# 2. Directions
### Steps to launch training for NVIDIA DGXA100 
...
### Run benchmark with docker for NVIDIA DGXA100 
```
    # nvidia-docker: Note that performance or functionality may vary from the tested SLURM instructions
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGXA100.sh. 
...
### Run benchmark with SLURM for NVIDIA DGXA100 
```
    # Data from DATADIR is mounted to /data/ in docker.
    # Launch configuration and system-specific hyperparameters for various system  are in config_DGXA100.sh.
    source config_DGXA100.sh 
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> sbatch -N 1 -t $WALLTIME run.sub
```
