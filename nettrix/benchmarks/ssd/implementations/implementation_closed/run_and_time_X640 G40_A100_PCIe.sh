...
PRETRAINED_BACKBONE=${PRETRAINED_BACKBONE:-"$(pwd)/pretrained/mxnet/resnet34-333f7ec4.pickle"}
...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  CMD=( mpirun --allow-run-as-root --bind-to none --np ${DGXNGPU} ./bind.sh python -u )
fi
