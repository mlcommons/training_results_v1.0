...
from REDACTED.mlperf.submissions.training.v1_0.models.unet3d.reference.data_loading import pytorch_loader
...
class SyntheticDataset(Dataset):
  ...
  def __init__(self,
    ...
    if scalar:
      ...
      y_shape = (channels_out,) + shape if layout == "NCDHW" else shape + (
          channels_out,)
...
def get_data_loaders(flags, num_shards):
  if flags.loader == "synthetic":
    ...
    train_dataset = pytorch_loader.PytTrain(x_train, y_train,
                                            **train_data_kwargs)
    val_dataset = pytorch_loader.PytVal(x_val, y_val)
  ...
  train_sampler = DistributedSampler(
      train_dataset, seed=flags.seed,
      drop_last=False) if num_shards > 1 else None
  # val_sampler = None
  # DistributedSampler(val_dataset, seed=flags.seed) if num_shards > 1 else None
  val_sampler = DistributedSampler(
      val_dataset, seed=flags.seed, drop_last=False) if num_shards > 1 else None
