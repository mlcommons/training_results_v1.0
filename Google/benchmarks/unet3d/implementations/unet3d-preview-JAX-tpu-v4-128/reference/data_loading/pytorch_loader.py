"""Reference implementation for pytorch_loader."""
...
class RandBalancedCrop:
  ...
  def __call__(self, data):
    ...
    if random.random() < self.oversampling:
      image, label, _ = self.rand_foreg_cropd(image, label)
    else:
      image, label, _ = self._rand_crop(image, label)
  ...
  def __call__(self, data):
    ...
    if random.random() < self.prob:
      ...
      noise = np.random.normal(
          loc=self.mean, scale=scale, size=image.shape).astype(image.dtype)
