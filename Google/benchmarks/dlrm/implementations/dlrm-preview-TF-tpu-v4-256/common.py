...
from REDACTED.tensorflow_models.official.utils.flags import core as flags_core
FLAGS = flags.FLAGS
...
def get_params() -> Dict[str, Any]:
  ...
  if params["use_synthetic_data"]:
    params["vocab_sizes"] = [fc.FAKE_DATA_VOCAB_SIZE for _ in FLAGS.vse]
  # Optimization
...
def define_dlrm_flags() -> None:
  ...
  # TODO: Merge flags with the low level runner.
  flags_core.define_base(
      num_gpu=False, distribution_strategy=True, model_dir=False)
  ...
  flags.DEFINE_float(
      ...
      default=0.01,
      help=flags_core.help_wrap("Adagrad initial accumulator values."))
  flags.DEFINE_integer(
      name="lr_warmup_steps",
      default=0,
      help=flags_core.help_wrap("Number of warmup steps in learning rate."))
  flags.DEFINE_integer(
      name="decay_steps",
      default=0,
      help=flags_core.help_wrap(
          "Number of decay steps used in polynomial decay."))
  flags.DEFINE_integer(
      name="decay_start_step",
      default=0,
      help=flags_core.help_wrap("Step to begin decay, if decay_steps > 0."))
  flags.DEFINE_integer(
      name="num_tpu_shards",
      default=8,
      help=flags_core.help_wrap("Number of shards (cores)."))
  flags.DEFINE_integer(
      name="eval_batch_size",
      short_name="ebs",
      default=16384,
      help=flags_core.help_wrap("Global batch size to use during eval."))
  flags.DEFINE_float(
      name="learning_rate",
      short_name="lr",
      default=0.01,
  ...
  flags.DEFINE_integer(
      ...
      short_name="es",
      default=5440,
      help=flags_core.help_wrap("The number of steps used to eval."))
  flags.DEFINE_integer(
  ...
  flags.DEFINE_integer(
      ...
      default=100,
      help=flags_core.help_wrap(
          "Number of training steps to run before communicating with host to "
          "send summaries."))
  flags.DEFINE_bool(
      name="use_cached_data",
      default=False,
      help=flags_core.help_wrap("If true, take a few samples and repeat."))
  flags.DEFINE_string(
      name="mode",
      default="train",
      help=flags_core.help_wrap("mode: train or eval"))
  flags.DEFINE_bool(
      name="use_ctl",
      default=False,
      help=flags_core.help_wrap(
  ...
  flags.DEFINE_bool(
      ...
      default=True,
      help=flags_core.help_wrap(
          "If true, data paths use terabyte format. Else kaggle."))
  # System params.
  flags.DEFINE_bool(
      name="pipeline_execution",
      default=False,
      help=flags_core.help_wrap(
          "If true, pipeline embedding execution with REDACTED."))
  flags.DEFINE_bool(
      name="use_batched_tfrecords",
      default=False,
      help=flags_core.help_wrap(
          "If true, use dataset of batched TFRecords, instead of csv."))
  flags.DEFINE_bool(
      name="bfloat16_grads_all_reduce",
      default=False,
      help=flags_core.help_wrap(
          "If true, use bfloat16 for all-reduce computation."))
  flags.DEFINE_string(
      name="data_cell",
      default="mb",
      help=flags_core.help_wrap(
          "Data cell. Path to data directory is determined dynamically."))
  flags.DEFINE_enum(
      "partition_strategy",
      default="mod",
      enum_values=["div", "mod"],
  ...
  flags.DEFINE_integer(
      ...
      short_name="de",
      default=4,
      help=flags_core.help_wrap("Embedding dimension."))
  flags.DEFINE_list(
      name="mlp_bottom",
      default="8, 4",
      help=flags_core.help_wrap(
          "Hidden layers for the bottom MLP. "
  ...
  flags.DEFINE_list(
      ...
      default="8, 8, 8, 8",
      help=flags_core.help_wrap(
          "Vocab sizes for each of the sparse features. The order agrees with "
          "the order of the input data."))
  flags.DEFINE_integer(
      name="num_dense_features",
      short_name="ndf",
      default=3,
      help=flags_core.help_wrap("Number of dense features."))
  flags.DEFINE_integer(
      name="num_tables_in_ec",
      default=26,
      help=flags_core.help_wrap(
          "Number of embedding tables in the embedding core."))

  flags_core.set_defaults(
      data_dir="/REDACTED/jn-d/home/tayo/tmp/dlrm/data/",
      batch_size=32)
