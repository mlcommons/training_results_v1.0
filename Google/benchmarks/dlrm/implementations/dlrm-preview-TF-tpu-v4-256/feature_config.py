...
class FeatureConfig(object):
  ...
  The embedding tables can be placed either in EmbeddingCore or REDACTED.
  In practice, large embedding tables are placed in EmbeddingCore while small
  ones are in REDACTED. FeatureConfig sorts the embedding table by its size
  ...
  def get_feature_tbl_config(self):
    ...
    for i in range(self._num_tables_in_ec):
      ...
      table_to_config_dict["tbl%02d" % i] = tpu_embedding.TableConfig(
          ...
          # TODO: Using the utils lr_fn leads to problems with embedding
