...
def rand_features(params, batch_size):
  ...
  num_ec_features = params["num_tables_in_ec"]
  num_tc_features = len(fc.CATEGORICAL_FEATURES) - num_ec_features
  if num_ec_features:
    features["cat-features"] = tf.random.uniform(
        shape=(batch_size, num_ec_features),
        maxval=fc.FAKE_DATA_VOCAB_SIZE,
        dtype=tf.int32)
  if num_tc_features:
    features["tc-features"] = tf.random.uniform(
        shape=(batch_size, num_tc_features),
        maxval=fc.FAKE_DATA_VOCAB_SIZE,
        dtype=tf.int32)
  return features
...
class CriteoTFRecordReader(object):
  ...
  def __init__(self,
    ...
    self._num_files = len(tf.gfile.Glob(file_path))
    self._parallelism = min(self._num_files // num_datasets, 8)

    tf.logging.info("%s files [parallelism = %s] in path: %s" % (
        self._num_files, self._parallelism, file_path))

    if self._num_files < num_datasets and not use_synthetic_data:
      raise ValueError("There must be at least one file per dataset.")

  def __call__(self, params):

    tf.set_random_seed(params["seed"])
    assert params["dataset_num_shards"] == self._num_datasets
    batch_size = params["batch_size"]
    if self._use_synthetic_data:
      ds = tf.data.Dataset.from_tensor_slices(
          rand_features(self._params, batch_size))
    ...
    if self._is_training:
      ds = ds.shuffle(self._num_files // self._num_datasets)
      ds = ds.repeat()

    ds = tf.data.TFRecordDataset(
        ds, buffer_size=64 * 1024 * 1024, num_parallel_reads=self._parallelism)
    ds = ds.map(_parse_fn, num_parallel_calls=self._parallelism)
    ds = ds.shuffle(256)
...
class CriteoTsvReader(object):
  ...
  def __init__(self,
    ...
    num_files = len(tf.gfile.Glob(file_path))
    if num_datasets > num_files and not use_synthetic_data:
      raise ValueError("There must be at least one file per dataset.")
    if num_datasets * parallelism > num_files and not use_synthetic_data:
      tf.logging.warning(
