...
from REDACTED.tensorflow.python.tpu import tensor_tracer
from REDACTED.tensorflow.python.tpu import topology as tf_topology
...
class TrainAndEvalRunner(object):
  ...
  def __init__(self,
    ...
    self.num_outfeed_threads = num_outfeed_threads
    self.host_id_map = {}
  ...
  def initialize(self,
    ...
    if self.eval_steps > 0:
      with output_graph.as_default():
        ...
        self.eval_output = [[] for _ in flatten_output]
  ...
  def train_and_eval(self,
    ...
    def outfeed_thread_fn(hosts):
      # Use Python concurrency to execute more frequent outfeed_dequeue ops.
      # This is useful when eval tensors are non-trivial in size.
      # Thread safety relies on atomicity of Python's list.append().
      for _ in range(self.num_outfeeds):
        for predict_item_idx in range(len(self.dequeue_ops)):
          for host_idx in hosts:
            host_outputs = self.dequeue_ops[predict_item_idx]
            cur_output = self.output_sess.run(host_outputs[host_idx])
            self.eval_output[predict_item_idx].append(list(cur_output))

    def create_outfeed_threads():

      def _host_range(t_idx):
        """Return list of host targets for each thread."""
        num_hosts = self.num_replicas // self.replicas_per_host
        per_thread_base = num_hosts // self.num_outfeed_threads
        discr = num_hosts % self.num_outfeed_threads
        if t_idx < discr:
          start_offset = t_idx
          added_host = 1
        else:
          start_offset = discr
          added_host = 0
        start_idx = (per_thread_base * t_idx) + start_offset
        num_thd_hosts = per_thread_base + added_host
        return list(range(start_idx, start_idx + num_thd_hosts))

      outfeed_threads = []
      for i in range(self.num_outfeed_threads):
        outfeed_threads.append(
            threading.Thread(target=outfeed_thread_fn, args=([_host_range(i)])))
      return outfeed_threads
    ...
    if self.eval_steps > 0:
      ...
      for cur_step in step_range:
        ...
        self.eval_output = [[] for _ in self.dequeue_ops]
        if self.num_outfeed_threads > 1:
          outfeed_threads = create_outfeed_threads()
          for thr in outfeed_threads:
            thr.start()
          for thr in outfeed_threads:
            thr.join()
        else:
          for _ in range(self.num_outfeeds):
            for i, t in enumerate(self.output_sess.run(self.dequeue_ops)):
              self.eval_output[i] += list(t)
        self.eval_output = tf.nest.pack_sequence_as(self.predict_output,
                                                    self.eval_output)
        if eval_finish_fn and not success and eval_finish_fn(
            cur_step, self.eval_output, summary_writer):
          success = True
