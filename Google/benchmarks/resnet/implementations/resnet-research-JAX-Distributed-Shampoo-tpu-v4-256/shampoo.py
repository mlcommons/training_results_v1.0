...
class _ShampooHyperParams:
  ...
  # Preconditioner data type (use bfloat16 to save memory). Experimentally
  # lower precision here doesn't seem to affect final solution quality.
  preconditioner_dtype: jnp.dtype
...
class Preconditioner:
  ...
  def preconditioned_grad(self, grad, preconditioners):
    ...
    for i, grad in enumerate(partitioned_grads):
      ...
      precond_grad = grad
      for j in range(rank):
        # Always cast to float32 for preconditioning.
        preconditioner = preconditioners_for_grad[j].astype(jnp.float32)
        precond_grad = jnp.tensordot(
            precond_grad, preconditioner, axes=[[0], [0]])
...
def matrix_inverse_pth_root(mat_g,
  ...
  Args:
    ...
    out_dtype: Dtype for output matrix.
  ...
  return resultant_mat_h.astype(out_dtype), error
...
class DistributedShampoo(OptimizerDef):
  ...
  def __init__(self,
    ...
    Args:
      ...
      preconditioner_dtype: Data type for preconditioner
  ...
  def init_param_state(self, param):
    ...
    if rank >= 1:
      ...
      preconditioners = [
          jnp.eye(s[0], dtype=hps.preconditioner_dtype) for s in shapes
      ]
  ...
  def compute_preconditioners_from_statistics(self, states, params, hps, step):
    ...
    def _matrix_inverse_pth_root(xs, ps):
      mi_pth_root = lambda x, y: matrix_inverse_pth_root(  # pylint: disable=g-long-lambda
          x,
          y,
          out_dtype=hps.preconditioner_dtype,
          ridge_epsilon=hps.matrix_eps)
      preconditioners, _ = jax.vmap(mi_pth_root)(xs, ps)
      return preconditioners

    if not hps.batch_axis_name:
      preconditioners = jax.pmap(_matrix_inverse_pth_root)(
      ...
      def _internal_inverse_pth_root_all():
        ...
        preconditioners = _matrix_inverse_pth_root(
            all_statistics[current_replica], all_exponents[current_replica])
        preconditioners = jax.lax.all_gather(preconditioners,
                                             hps.batch_axis_name)
        preconditioners_flat = _unbatch(preconditioners)
        return preconditioners_flat

      if hps.preconditioning_compute_steps == 1:
        preconditioners_flat = _internal_inverse_pth_root_all()
      else:
        # Passing statistics instead of preconditioners as they are similarly
        # shaped tensors. Actual data is ignored.
        preconditioners_init = [
            p.astype(hps.preconditioner_dtype) for p in packed_statistics
        ]
        init_state = preconditioners_init
        perform_step = step % hps.preconditioning_compute_steps == 0
        preconditioners_flat = list(self.fast_cond(
            perform_step, _internal_inverse_pth_root_all, init_state))

    new_preconditioners_flat = []
    for p, shape in zip(preconditioners_flat, original_shapes):
      new_preconditioners_flat.append(p[:shape[0], :shape[1]])
