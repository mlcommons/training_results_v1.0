...
def load_split(batch_size, train, dtype=tf.float32, image_size=IMAGE_SIZE,
               transpose_images=False, space_to_depth=False):
  ...
  Args:
    ...
    transpose_images: Whether to use the "double transpose trick".
    space_to_depth: Whether to space-to-depth transform input images.
  Returns:
    A `tf.data.Dataset`.
  """
  if train:
    ...
    split = 'train[{}:{}]'.format(int(start), int(start + split_size))
  else:
    split_size = EVAL_IMAGES / jax.host_count()
    start = jax.host_id() * split_size
    split = 'validation[{}:{}]'.format(int(start), int(start + split_size))
  ...
  def decode_example(example):
    ...
    return {'image': image, 'label': tf.cast(example['label'], tf.int32)}

  def post_process_batch(batch):
    """Apply transpose/space-to-depth."""
    images, labels = batch['image'], batch['label']
    if space_to_depth:
      images = tf.reshape(
          images, [-1, image_size // 2, 2, image_size // 2, 2 * 3])
      images = tf.transpose(images, [0, 1, 3, 2, 4])
      images = tf.reshape(
          images, [-1, image_size // 2, image_size // 2, 4 * 3])
    if transpose_images:
      # apparently [1, 2, 0, 3] is better for per-device batch <= 8?
  ...
  if train:
    if split_size < 16384:
      ds = ds.cache()
    ds = ds.concatenate(
        ds.shuffle(16384, seed=0, reshuffle_each_iteration=True).repeat())

  ds = ds.map(decode_example, 64)
  ds = ds.batch(batch_size, drop_remainder=train)

  ds = ds.map(post_process_batch, 64)
