...
FLAGS = flags.FLAGS
flags.DEFINE_bool("enable_block_matmul", False,
                  "Whether to enable block matmul.")
flags.DEFINE_bool("stack_block_matmul_variables", False,
                  "Whether to merge block matmul variables for all layers")

flags.DEFINE_integer("shampoo_block_size", 512, "Block matmul block size.")


BATCH_MATMUL_VAR_COLLECTION = "BATCH_MATMUL_VAR_COLLECTION"
...
def dense_layer_3d(input_tensor,
  ...
  Args:
    ...
    partition_dim: Which dimension to shard.
    ...
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      if num_partitions > 1:
        w = xla_sharding.split(
            w, partition_dim, num_partitions, use_sharding_op=True)
    b = tf.get_variable(
        name="bias",
...
def get_block_mat_dims(input_dim, output_dim, block_dim):
  """Get number of input and output blocks."""
  # Number of input and output blocks.
  w_im = input_dim // block_dim
  w_om = output_dim // block_dim
  # Add padding if input_dim / output_dim is not divisible by block_dim.
  if input_dim % block_dim != 0:
    w_im += 1
  if output_dim % block_dim != 0:
    w_om += 1
  return w_im, w_om


def block_matmul_dense_layer_2d(input_tensor,
                                layer_idx,
                                total_layers,
                                output_size,
                                initializer,
                                ...
                                num_partitions=0,
                                name=None):
  """A dense layer with block matmul implementation.
  ...
  if num_partitions > 1:
    raise NotImplementedError(
        "No Spatial partitioning for Shampoo optimizer with blocking")

  bsz1 = get_shape_list(input_tensor)[0]
  bsz2 = get_shape_list(input_tensor)[1]
  last_dim = get_shape_list(input_tensor)[-1]

  w_im, w_om = get_block_mat_dims(
      input_dim=last_dim,
      output_dim=output_size,
      block_dim=FLAGS.shampoo_block_size)

  with tf.variable_scope(name, dtype=input_tensor.dtype):
    if FLAGS.stack_block_matmul_variables:
      w = tf.get_variable(
          name="kernel",
          shape=[
              total_layers * w_im * w_om, FLAGS.shampoo_block_size,
              FLAGS.shampoo_block_size
          ],
          initializer=initializer,
          collections=[
              BATCH_MATMUL_VAR_COLLECTION, tf.GraphKeys.GLOBAL_VARIABLES
          ])
      w = tf.reshape(w, [
          total_layers, w_im * w_om,
          FLAGS.shampoo_block_size, FLAGS.shampoo_block_size
      ])
      with tf.variable_scope("layer_%d" % layer_idx):
        w = tf.gather(w, layer_idx)
    else:
      with tf.variable_scope("layer_%d" % layer_idx):
        w = tf.get_variable(
            name="kernel",
            shape=[
                w_im * w_om, FLAGS.shampoo_block_size, FLAGS.shampoo_block_size
            ],
            initializer=initializer,
            collections=[
                BATCH_MATMUL_VAR_COLLECTION, tf.GraphKeys.GLOBAL_VARIABLES
    ...
    with tf.variable_scope("layer_%d" % layer_idx):
      w = tf.reshape(w, [w_om,
                         w_im * FLAGS.shampoo_block_size,
                         FLAGS.shampoo_block_size])
      if last_dim % FLAGS.shampoo_block_size != 0:
        w = tf.slice(w, [0, 0, 0], [w_om, last_dim, FLAGS.shampoo_block_size])
      ret = tf.einsum("abn,mnk->abmk", input_tensor, w)
      ret = tf.reshape(ret, [bsz1, bsz2, -1])

      if output_size % FLAGS.shampoo_block_size != 0:
        out_shape = [bsz1, bsz2, output_size]
        ret = tf.slice(ret, [0, 0, 0], out_shape)
...
def dense_layer_2d(input_tensor,
  ...
  Returns:
    float logits Tensor.
  """
  if FLAGS.enable_block_matmul:
    return block_matmul_dense_layer_2d(input_tensor,
                                       layer_idx,
                                       total_layers,
                                       output_size,
                                       initializer,
...
def attention_layer(from_tensor,
  ...
  # TODO automate this optimization in xla
    ...
    with tf.variable_scope("layer_common", reuse=tf.AUTO_REUSE):
      ...
      with tf.variable_scope("attention"):
        ...
        with tf.variable_scope("output"):
          attention_output = dense_layer_3d_proj(
              ...
              create_initializer(initializer_range), None, 0, num_partitions,
              "dense")
      ...
      with tf.variable_scope("intermediate"):
        intermediate_output = dense_layer_2d(
            ...
            create_initializer(initializer_range), intermediate_act_fn, 1,
            num_partitions, "dense")
