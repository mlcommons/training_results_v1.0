...
from REDACTED.cloud_tpu.models.retinanet.object_detection import argmax_matcher
from REDACTED.cloud_tpu.models.retinanet.object_detection import box_list
from REDACTED.cloud_tpu.models.retinanet.object_detection import faster_rcnn_box_coder
from REDACTED.cloud_tpu.models.retinanet.object_detection import preprocessor
from REDACTED.cloud_tpu.models.retinanet.object_detection import region_similarity_calculator
from REDACTED.cloud_tpu.models.retinanet.object_detection import target_assigner
from REDACTED.cloud_tpu.models.retinanet.object_detection import tf_example_decoder
...
def encode_labels(gt_boxes, gt_labels, use_spatial_partitioning):
  ...
  if use_spatial_partitioning:
    ...
    transposed_classes = {}
    transposed_boxes = {}
    for i, j in enumerate(tf.split(encoded_classes, num_elements, 0)):
      transposed_classes[i] = tf.transpose(
          tf.reshape(
              tf.cast(j, tf.int32), [
                  ssd_constants.NUM_DEFAULTS[i], ssd_constants.FEATURE_SIZES[i],
                  ssd_constants.FEATURE_SIZES[i]
              ]), [1, 2, 0])
    for i, j in enumerate(tf.split(encoded_boxes, num_elements, 0)):
      transposed_boxes[i] = tf.transpose(
          tf.reshape(j, [
              ssd_constants.NUM_DEFAULTS[i], ssd_constants.FEATURE_SIZES[i],
              ssd_constants.FEATURE_SIZES[i], 4
          ]), [1, 2, 0, 3])
    return transposed_classes, transposed_boxes, num_matched_boxes
  else:
  ...
  def _parse_example(self, data):
    ...
    with tf.name_scope('augmentation'):
      ...
      if self._is_training:
        ...
        # TODO: Investigate the parameters for color jitter.
        image = color_jitter(
