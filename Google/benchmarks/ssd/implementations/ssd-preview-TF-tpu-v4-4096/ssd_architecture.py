...
from REDACTED.mlperf.submissions.training.v1_0.models.ssd import ssd_checkpoint_map
from REDACTED.mlperf.submissions.training.v1_0.models.ssd import ssd_constants
from REDACTED.mlperf.submissions.training.v1_0.models.util import image_util
...
def conv0_space_to_depth(inputs, data_format='channels_last'):
  ...
  kernel = tf.pad(
      ...
      mode='CONSTANT',
      constant_values=0.)
  # Transform kernel follows the space-to-depth logic: http://REDACTED/_9YvHW96xPJ
  kernel = tf.reshape(
      kernel,
...
def ssd(features, params, is_training_bn=False):
  ...
  with tf.variable_scope(
    ...
    if params['map_checkpoint']:
      print('Mapping PyT checkpoint to TF and quitting')
      ssd_checkpoint_map.checkpoint_map(
          ssd_constants.PYT_PICKLE_FILE,
          ssd_constants.TF_CHECKPOINT_FILE)
