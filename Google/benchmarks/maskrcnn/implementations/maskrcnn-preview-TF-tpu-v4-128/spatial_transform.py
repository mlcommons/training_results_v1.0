...
def fused_transpose_and_space_to_depth(
    images, image_size, block_size=2,
    transpose_input=True, transpose_img_dimensions_last=False):
  ...
  Args:
    ...
    transpose_input: A boolean to indicate if the images tensor should be
      transposed.
    transpose_img_dimensions_last: A boolean to indicate if images tensor should
      be transposed with image dimensions last. If `True`, the value of
      `transpose_input` will be ignored.
  ...
  images = tf.reshape(images,
                      [batch_size, h//block_size, block_size,
                       w//block_size, block_size, c])
  if transpose_img_dimensions_last:
    images = tf.transpose(images, [0, 2, 4, 5, 1, 3])
    images = tf.reshape(
        images,
        [batch_size, c * (block_size**2), h // block_size, w // block_size])
    images = tf.reshape(images, [batch_size, c * (block_size**2), -1])
