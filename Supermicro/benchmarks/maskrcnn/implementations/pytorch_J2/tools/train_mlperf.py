...
def mlperf_test_early_exit(iteration, iters_per_epoch, tester, model, distributed, min_bbox_map, min_segm_map):
    ...
    if iteration > 0 and (iteration + 1)% iters_per_epoch == 0:
        ...
        model.train()
    elif iteration % 3 == 2: # do finished check every 3 iterations
...
def train(cfg, local_rank, distributed, random_number_generator=None, seed=None):
    ...
    if cfg.USE_CUDA_GRAPH:
        ...
        for i, shape in enumerate(shapes):
            ...
            sample_args = (dummy_batch.clone(),)

            forward_fn = "graph_forward_%d" % (i+1)
            if i == 0:
                model.backbone = graph(model.backbone,
                                       sample_args,
                                       graph_stream=graph_stream,
                                       warmup_only=True,
                                       overwrite_fn='eager_forward')
                model.backbone, pool_id = graph(model.backbone,
                                                sample_args,
                                                graph_stream=graph_stream,
                                                warmup_only=False,
                ...
                model.backbone = graph(model.backbone,
                                       ...
                                       overwrite_fn=forward_fn,
                                       use_pool_id=pool_id)
            graphed_forwards[dummy_shape] = getattr(model.backbone, forward_fn)
        ...
        class GraphedWrapper(torch.nn.Module):
            ...
            def forward(self, im_tensor):
                if not self.training:
                    return self.model_segment.eager_forward(im_tensor)
                else:
                    shape = tuple(list(im_tensor.shape))
                    if shape in self.graphed_forwards:
                        return self.graphed_forwards[shape](im_tensor)
                    else:
                        return self.model_segment.eager_forward(im_tensor)

        model.backbone = GraphedWrapper(model.backbone, graphed_forwards)
    else:
        shapes = None
    # ------------------
