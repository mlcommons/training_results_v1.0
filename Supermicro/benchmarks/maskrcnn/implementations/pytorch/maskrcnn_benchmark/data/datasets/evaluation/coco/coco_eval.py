...
def prepare_for_evaluation(iou_types, eval_prep_args, coco, eval_segm_numprocs, eval_mask_virtual_paste, output_folder):
    ...
    coco_results = {}
    with TimedSection("Prepping took %.3f seconds total"):
...
def all_gather_prep_work(coco_results):
    if "segm" in coco_results: coco_results["segm"] = all_gather(coco_results["segm"])
    if "bbox" in coco_results: coco_results["bbox"] = all_gather(coco_results["bbox"])
    if "keypoints" in coco_results: coco_results["keypoints"] = all_gather(coco_results["keypoints"])
    return coco_results
