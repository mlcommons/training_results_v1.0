...
#             y_pred = F.softmax(y_pred, axis=channel_axis)
#         elif self.use_argmax:
#             y_pred = F.argmax(y_pred, axis=channel_axis)
#
#         if self.to_onehot_y:
#             y_true = to_one_hot(F, y_true, channel_axis, num_pred_ch)
#
#         if self.to_onehot_x:
#             y_pred = to_one_hot(F, y_pred, channel_axis, num_pred_ch)
#
#         if not self.include_background:
#             assert num_pred_ch > 1, \
#                 f"To exclude background the prediction needs more than one channel. Got {num_pred_ch}."
