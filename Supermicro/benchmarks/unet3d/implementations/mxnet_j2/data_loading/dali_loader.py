...
class ExternalInputIterator(object):
    def __init__(self, batch_size, device_id, shard_id, num_shards, filelist, seed, in_gpu=False, name=""):
        ...
        self.files = self.filelist #[self.data_set_len * device_id // num_gpus :
    ...
    def __next__(self):
        ...
        #print(f"{self.device_id}:  picking up {self.i} . {self.order[self.i]}")
...
class TrainNumpyPipeline(BasicPipeline):
    def __init__(self, flags, batch_size, image_list, label_list, num_shards=1, device_id=0, shard_id=0, seed=0, image_iterator =None, label_iterator=None, ):
...
class RateMatchInputIterator:
    def __init__(self, iterator, input_batch_size, output_batch_size, device_id, flags):
        ...
        self._input_buffer = None
        if flags.use_spatial_loader:
            flags.input_shape[0] = flags.input_shape[0] // flags.spatial_group_size  
        self._output_buffer = get_buffer(output_batch_size, flags.input_shape, device_id)
        self._input_offset = 0
        self._output_offset = 0
        self._batch_count = 0
        self.spatial_loader = flags.use_spatial_loader
    ...
    def batch_size(self):
        if self.spatial_loader:
            return self.iterator.iterator.batch_size
        else:
            return self.iterator.batch_size 
    ...
    def epoch_size(self, pipeline_id=0):
        if self.spatial_loader: 
            return self.iterator.iterator._pipes[pipeline_id].epoch_size
        else:
            return self.iterator._pipes[pipeline_id].epoch_size

    def shard_size(self):
        if self.spatial_loader:  
            return self.iterator.iterator.shard_size
        else:
            return self.iterator.shard_size 

    def __len__(self):
        if self.spatial_loader:  
            return self.iterator.iterator.shard_size // self.output_batch_size
        else:
            return self.iterator.shard_size // self.output_batch_size
...
def get_dali_loader(flags,
    ...
    if mode == "train" and flags.input_batch_multiplier > 1:
        if flags.spatial_group_size > 1 and flags.use_spatial_loader:
            dali_iter = ScatterInputIterator(iterator=dali_iter,
                                             batch_size=input_batch_size,
                                             device_id=device_id,
                                             shard_id = shard_id,
                                             flags=flags, global_rank=global_rank)
