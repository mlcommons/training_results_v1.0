...
class DaliPipeline(nvidia.dali.pipeline.Pipeline):
    def __init__(self, *,
        ...
        if synthetic_seq_len is None:
            if in_mem_file_list:
                self.read = ops.FileReader( name="Reader", dont_use_mmap=True, 
            ...
            self.read = ops.FileReader(name="Reader", dont_use_mmap=True, pad_last_batch=(pipeline_type == 'val'), device="cpu", file_root=file_root, file_list="/workspace/rnnt/rnnt_dali.file_list.synth", shard_id=shard_id,
                                       num_shards=n_shards, shuffle_after_epoch=shuffle)
    ...
    def from_config(cls, pipeline_type, device_id, batch_size, file_root: str, sampler, config_data: dict,
                    ...
                    in_mem_file_list=True, pre_sort=False):
