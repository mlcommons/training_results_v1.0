 ...
 public:
  ...
  void run() {
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      download_tensor(h_model_indices[i], this->infrequent_embeddings[i].model_indices_,
                      this->stream);
      download_tensor(h_model_indices_offsets[i],
                      this->infrequent_embeddings[i].model_indices_offsets_, this->stream);
    }
 ...
 public:
  ...
  void run() {
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      download_tensor(h_network_indices[i], this->infrequent_embeddings[i].network_indices_,
                      this->stream);
      download_tensor(h_network_indices_offsets[i],
                      this->infrequent_embeddings[i].network_indices_offsets_, this->stream);
    }
  ...
  void run() {
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      download_tensor(h_frequent_sample_indices[i],
                      this->frequent_embeddings[i].frequent_sample_indices_, this->stream);
    }
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      CK_CUDA_THROW_(
          cudaMemcpyAsync(&num_frequent_sample_indices,
                          this->frequent_embeddings[i].d_num_frequent_sample_indices_.get_ptr(),
 ...
 public:
  ...
  void run() {
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      download_tensor(h_model_cache_indices[i], this->frequent_embeddings[i].model_cache_indices_,
                      this->stream);
      download_tensor(h_model_cache_indices_offsets[i],
                      this->frequent_embeddings[i].model_cache_indices_offsets_, this->stream);
    }
  ...
  void run() {
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      download_tensor(h_network_cache_indices[i],
                      this->frequent_embeddings[i].network_cache_indices_, this->stream);
      download_tensor(h_network_cache_indices_offsets[i],
                      this->frequent_embeddings[i].network_cache_indices_offsets_, this->stream);
      h_network_cache_mask[i].resize(this->config.num_frequent);
      CK_CUDA_THROW_(cudaMemcpyAsync(
          h_network_cache_mask[i].data(),
          reinterpret_cast<uint8_t*>(this->frequent_embeddings[i].network_cache_mask_.get_ptr()),
...
// Various numbers of instances to test the sort in calculate_network_indices
static const HybridEmbeddingConfig<uint32_t> config_1gpu = {1, 1, 10, 128, 1000, 128, 0.5f};
static const HybridEmbeddingConfig<uint32_t> config_2gpus = {1, 2, 10, 128, 1000, 128, 0.5f};
static const HybridEmbeddingConfig<uint32_t> config_3gpus = {1, 3, 10, 128, 1000, 128, 0.5f};
static const HybridEmbeddingConfig<uint32_t> config_9gpus = {1, 9, 10, 128, 1000, 128, 0.5f};
...
TEST(hybrid_embedding_network_indices_test, uint32_float_64_1gpu) {
  CalculateNetworkIndicesTest<uint32_t, float>(config_1gpu, 64).run();
}

TEST(hybrid_embedding_network_indices_test, uint32_float_64_2gpus) {
  CalculateNetworkIndicesTest<uint32_t, float>(config_2gpus, 64).run();
}

TEST(hybrid_embedding_network_indices_test, uint32_float_96_3gpus) {
  CalculateNetworkIndicesTest<uint32_t, float>(config_3gpus, 96).run();
}

TEST(hybrid_embedding_network_indices_test, uint32_float_144_9gpus) {
  CalculateNetworkIndicesTest<uint32_t, float>(config_9gpus, 144).run();
}
