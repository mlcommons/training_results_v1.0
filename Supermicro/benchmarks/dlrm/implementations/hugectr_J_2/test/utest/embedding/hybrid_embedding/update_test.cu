...
class InfrequentUpdateTest : public HybridEmbeddingUnitTest<dtype, emtype> {
 ...
 public:
  ...
  void run() {
    ...
    if (!single_node) cpu_embedding.backward_a2a_messages();
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      if (single_node) {
        ...
        this->infrequent_embeddings[i].update_model_direct(this->config.lr, this->stream);
      } else {
        upload_tensor(cpu_embedding.backward_received_messages[i], received_messages[i],
                      this->stream);
        this->infrequent_embeddings[i].update_model(received_messages[i].get_ptr(), this->config.lr,
                                                    this->stream);
      }
    ...
    for (size_t i = 0; i < this->num_instances; i++) {
      ...
      if (single_node)
        ...
        upload_tensor(cpu_embedding.reduced_gradients,
                      this->frequent_embeddings[i].get_gradients(), this->stream);
    }
    for (size_t i = 0; i < this->num_instances; i++) {
      if (single_node) {
        ...
        this->frequent_embeddings[i].update_model(this->config.lr, this->stream);
      }
    }
    for (size_t i = 0; i < this->num_instances; i++) {
