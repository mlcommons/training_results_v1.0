 ...
 public:
  void build_model() {
    ...
    for (size_t i = 0; i < num_instances; i++) {
      model_list.emplace_back(CommunicationType::IB_NVLink, i, num_instances_per_node_list,
  ...
  void build_frequent() {
    ...
    std::vector<const float *> h_frequent_vectors_pointers(num_instances);
    for (uint32_t i = 0; i < num_instances; i++) {
      h_frequent_vectors_pointers[i] = frequent_embeddings[i].frequent_embedding_vectors_.get_ptr();
    }
    for (uint32_t i = 0; i < num_instances; i++) {
      CK_CUDA_THROW_(cudaMemcpyAsync(frequent_embeddings[i].embedding_vectors_pointers_.get_ptr(),
                                     h_frequent_vectors_pointers.data(),
                                     num_instances * sizeof(float *), cudaMemcpyHostToDevice,
                                     stream));
    }
  }
  ...
  void build_infrequent() {
    ...
    std::vector<const float *> h_infrequent_vectors_pointers(num_instances);
    for (uint32_t i = 0; i < num_instances; i++) {
      h_infrequent_vectors_pointers[i] =
          infrequent_embeddings[i].infrequent_embedding_vectors_.get_ptr();
    }
    for (uint32_t i = 0; i < num_instances; i++) {
      CK_CUDA_THROW_(cudaMemcpyAsync(infrequent_embeddings[i].embedding_vectors_pointers_.get_ptr(),
                                     h_infrequent_vectors_pointers.data(),
                                     num_instances * sizeof(float *), cudaMemcpyHostToDevice,
                                     stream));
    }
  }
