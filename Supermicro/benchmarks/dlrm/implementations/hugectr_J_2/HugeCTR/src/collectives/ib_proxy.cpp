...
namespace HugeCTR 
{
      ...
      P2MHierA2AvBufInit& out)
  {
    ...
    size_t h_max_send_size_per_dest = in.h_max_send_size_ / (num_procs_*num_gpus_);
    size_t h_max_recv_size_per_dest = in.h_max_recv_size_ / (num_procs_*num_gpus_);

    // Initialize send/recv sizes to max
    for (size_t i = 0; i < num_procs_*num_gpus_; i++) {
      send_sizes_[i] = h_max_send_size_per_dest;
      recv_sizes_[i] = h_max_recv_size_per_dest;
    }
    ...
    for (size_t n = 0; n < num_procs_; n++) {
      for (size_t g = 0; g < num_gpus_; g++) {
        ...
        size_t offset = (n*num_gpus_ + g)*h_max_send_size_per_dest;
        sge.addr = (uintptr_t)((char*)in.d_send_ptrs_ + offset); 
        sge.length = h_max_send_size_per_dest;
    ...
    for (size_t n = 0; n < num_procs_; n++) {
      for (size_t g = 0; g < num_gpus_; g++) {
        size_t offset = (my_proc_*num_gpus_ + g)*h_max_recv_size_per_dest;
  ...
  void IbvProxy::ARCollContext::init_buf(const M2PARBufInit& in, P2MARBufInit& out)
  {
    ...
    *h_rs_cmd_ = 0;
    ...
    CK_CUDA_THROW_(cudaMalloc(&d_gdr_ag_storage_, 2*gpu_page_size));
    CK_CUDA_THROW_(cudaMemset(d_gdr_ag_storage_, 0, 2*gpu_page_size));
    d_gdr_ag_cmd_ = (size_t*)(((CUdeviceptr)d_gdr_ag_storage_ + gpu_page_size - 1) & gpu_page_mask);

    // TODO: check if this needs to be called only once per process
    gdr_ = gdr_open();
    PROXY_ASSERT(gdr_);
    {
      auto ret = gdr_pin_buffer(gdr_, (CUdeviceptr)(d_gdr_ag_cmd_), gpu_page_size, 0, 0, &gdr_mh_);
      PROXY_ASSERT(ret == 0);
    }
    {
  ...
  IbvProxy::ARCollContext::~ARCollContext()
  {
    ...
    if (d_gdr_ag_storage_) { cudaFree(d_gdr_ag_storage_); }
