...
namespace HugeCTR 
{
  ...
  void IbComm::detect_ib_devs()
  {
    ...
    for (int d = 0; d < num_devices; d++) {
      ...
      if((dev_list[d]->node_type != IBV_NODE_RNIC) && 
          (dev_list[d]->node_type != IBV_NODE_CA)) {
        continue;
      }
  ...
  void IbComm::print_obj(size_t my_rank, std::string obj_name, hwloc_obj_t obj) 
  {
    ...
    if (obj->type == HWLOC_OBJ_PCI_DEVICE) {
      std::cout << obj_name << ":PCIeDevice " << obj->gp_index << " " << obj << " " << obj->depth << " " << obj->attr->pcidev.dev << std::endl; 
    }
    else if (obj->type == HWLOC_OBJ_OS_DEVICE) {
      std::cout << obj_name << ":OSdev " << obj->gp_index << " " << obj << " " << obj->depth << " " <<  obj->name << " " << obj->attr->osdev.type << std::endl;
    }
    else if (obj->type == HWLOC_OBJ_BRIDGE) {
      std::cout << obj_name << ":PCIeBridge " << obj->gp_index << " " << obj << " " << obj->depth << std::endl; 
    }
    else {
      std::cout << obj_name << ":Unknown " << obj->gp_index << " " << obj << " " << obj->depth << std::endl;
    }
  }
  ...
  size_t IbComm::calculate_pcie_hier_distance(size_t my_rank, hwloc_obj_t obj1, hwloc_obj_t obj2)
  {
    ...
    auto are_bridges = [is_bridge](hwloc_obj_t obj1, hwloc_obj_t obj2) { return is_bridge(obj1) && is_bridge(obj2); };
  ...
  void IbComm::calculate_gpu_nic_affinity()
  {
    ...
    for (size_t g = 0; g < num_gpus_; g++) {
      ...
      for (size_t n = 0; n < ib_dev_list_.size(); n++) {
        ...
        gpu_nic_dist[g][n] = calculate_pcie_hier_distance(my_proc_, gpu_obj, ib_dev_list_[n].hwloc_obj);
      }
    }
    ...
    gpu_nic_affinity_names = (char (**)[IBV_SYSFS_NAME_MAX])malloc(sizeof(char(*)[IBV_SYSFS_NAME_MAX]) * num_procs_);
    for (size_t r = 0; r < num_procs_; r++) {
      gpu_nic_affinity_names[r] = (char (*)[IBV_SYSFS_NAME_MAX])malloc(sizeof(char[IBV_SYSFS_NAME_MAX]) * num_gpus_);
    }
    for (size_t g = 0; g < num_gpus_; g++) {
      auto ib_dev = ib_dev_list_[gpu_nic_affinity_[g]];
    ...
    for (size_t r = 0; r < num_procs_; r++) {
      CK_MPI_THROW_(MPI_Bcast(gpu_nic_affinity_names[r], num_gpus_*sizeof(char[IBV_SYSFS_NAME_MAX]), MPI_BYTE, r, MPI_COMM_WORLD));
    }
    ...
    for (size_t r = 0; r < num_procs_; r++) {
      for (size_t g = 0; g < num_gpus_; g++) {
        ...
        if (my_ib_name != remote_ib_name) {
          std::cout << "WARNING: Mismatch in mellanox dev names. " << g << " " << my_proc_ << ":" << my_ib_name <<  " " << r << ":" << remote_ib_name << std::endl;
          std::cout << "WARNING: Non uniform cluster detected. Performance maybe impacted" << std::endl;
        }
      }
    }
  ...
  IbComm::HierA2ACollContext::HierA2ACollContext(IbComm* comm)
  {
    ...
    *h_recv_cmd_ptr_ = 1;
    ...
    for (size_t g = 0; g < num_gpus; g++) {
      ...
      size_t init_value = 2;
      CK_CUDA_THROW_(cudaMemcpy(d_send_cmd_[g], &init_value, sizeof(size_t),
            cudaMemcpyHostToDevice));

      CK_CUDA_THROW_(cudaMalloc((void**)&d_ibv_atomic_[g], 
            MAX_IBV_DEST*sizeof(size_t)));
      size_t atomic_init_values[MAX_IBV_DEST];
      std::fill_n(atomic_init_values, MAX_IBV_DEST, 1);
      CK_CUDA_THROW_(cudaMemcpy(d_ibv_atomic_[g], atomic_init_values, 
            MAX_IBV_DEST*sizeof(size_t), cudaMemcpyHostToDevice));

      CK_CUDA_THROW_(cudaMalloc((void**)&d_ibv_atomic_recv_[g], 
            MAX_IBV_DEST*sizeof(size_t)));
  ...
  IbComm::HierA2ACollContext::~HierA2ACollContext()
  {
    ...
    if (d_ibv_atomic_recv_) { 
      for (size_t g = 0; g < num_gpus; g++) { cudaFree(d_ibv_atomic_recv_[g]); }
      delete d_ibv_atomic_recv_;
    }
    if (d_ibv_atomic_) { 
      for (size_t g = 0; g < num_gpus; g++) { cudaFree(d_ibv_atomic_[g]); }
      delete d_ibv_atomic_;
    }
    if (d_send_cmd_) { 
      for (size_t g = 0; g < num_gpus; g++) { cudaFree(d_send_cmd_[g]); }
      delete d_send_cmd_;
    }
    if (cmd_storage_) { cudaFree(cmd_storage_); }
  }

  IbComm::HierA2ACollContextPerGPU::~HierA2ACollContextPerGPU()
  {
    if (d_send_ptrs_) { free(d_send_ptrs_); }
    if (d_recv_ptrs_) { free(d_recv_ptrs_); }
    if (d_send_sizes_copy_) { cudaFree(d_send_sizes_copy_); }
  }
  ...
  void IbComm::set_a2a_coll_buf(HierA2ACollHandle coll,
      void** send_ptrs, const size_t* send_max_size,
  ...
  void IbComm::set_a2a_coll_buf(HierA2AvCollHandle coll,
      void* send_ptrs, const size_t send_max_size,
      void* recv_ptrs, const size_t recv_max_size,
      size_t device_id)
  {
    ...
    CK_CUDA_THROW_(cudaMalloc((void**)(&gpu_ctx.d_send_sizes_copy_), sizeof(size_t)*num_gpus_*num_procs_));
  ...
  void IbComm::update_a2a_coll_sizes(HierA2AvCollHandle coll,
      const size_t* d_send_sizes, const size_t* d_recv_sizes,
      cudaStream_t dep_stream, size_t device_id)
  {
    ...
    update_sizes<<<n_blocks, MAX_TPB, 0, gpu_ctx.stream_>>>(
        gpu_ctx.h_send_sizes_, gpu_ctx.h_recv_sizes_,
        gpu_ctx.d_send_sizes_copy_,
        d_send_sizes, d_recv_sizes, num_procs_*num_gpus_);
  }

  // Local first distribution TODO: node first might be efficient
  static __global__ void update_pre_intra_sizes(
      size_t* __restrict__ h_send_sizes, 
      ...
      cudaStream_t dep_stream, size_t device_id)
  {
    ...
    update_pre_intra_sizes<<<num_procs_, num_gpus_, 0, gpu_ctx.stream_>>>(
        gpu_ctx.h_send_sizes_, gpu_ctx.d_send_sizes_copy_, d_pre_intra_send_sizes,
        device_id, num_gpus_, num_procs_);
  }
  ...
  template<typename T>
  static __global__ void copy_local(
      const T* __restrict__ input_, 
      T* __restrict__ output_, 
      size_t size)
  {
    for (size_t i = blockIdx.x*blockDim.x + threadIdx.x; i < size; i += blockDim.x*gridDim.x)
    {
  ...
  template<typename T>
  static __global__ void copy_local_segmented(
      const T* __restrict__ input_, 
      T* __restrict__ output_, 
      const size_t* __restrict__ sizes, 
      int num_segments, size_t offset)
  {
  ...
  static __global__ void wait_completion(size_t* d_ibv_cmd, size_t* atomic, 
      int nDest, int myDest, int device_id)
  {
    if ((threadIdx.x < nDest) && (threadIdx.x != myDest)) {
      ...
      while(*((volatile size_t*) &atomic[threadIdx.x]) < (curr_count - 1)) 
      {
        ...
        //   printf("wait completion expected: %llu %llu, got %llu from_dest %d my_dest %d %d n_dest %d\n", 
        //     curr_count, (curr_count - 1), atomic[threadIdx.x], threadIdx.x, myDest, device_id, nDest); 
        //   s = clock64();
        // }
      }
    }
  ...
  template<typename T>
  void IbComm::post_send_command_a2a<T>(HierA2ACollHandle coll, 
      cudaStream_t dep_stream, 
      size_t device_id)
  {
    ...
    CK_CUDA_THROW_(cudaEventRecord(gpu_ctx.event_, dep_stream));
    CK_CUDA_THROW_(cudaStreamWaitEvent(gpu_ctx.stream_, gpu_ctx.event_));
    ctx.barrier_->sync_all_gpus_report_host_and_inc(ctx.d_send_cmd_[device_id], 
        ctx.h_recv_cmd_ptr_, 
    ...
    copy_local<T><<<96, 1024, 0, gpu_ctx.stream_>>>(
        (T*)gpu_ctx.d_send_ptrs_[my_proc_], 
        (T*)gpu_ctx.d_recv_ptrs_[my_proc_],
        num_elems);
    wait_completion<<<1, 32, 0, gpu_ctx.stream_>>>(
        ctx.d_send_cmd_[device_id], ctx.d_ibv_atomic_[device_id], 
        num_procs_, my_proc_, device_id);
  }

  template <typename T>
  void IbComm::post_send_command_a2a<T>(HierA2AvCollHandle coll, 
      cudaStream_t dep_stream, 
      size_t device_id)
  {
    auto& ctx = *hier_a2a_v_coll_ctx_[coll];
    ...
    copy_local_segmented<T><<<96, 1024, 0, gpu_ctx.stream_>>>(
        ...
        (T*)gpu_ctx.d_recv_ptrs_[0] + (my_proc_*num_gpus_*offset),
        copy_sizes,
        num_gpus_, offset);
    wait_completion<<<1, 32, 0, gpu_ctx.stream_>>>(
        ctx.d_send_cmd_[device_id], ctx.d_ibv_atomic_[device_id], 
  ...
  void IbComm::wait_global_recv_async(HierA2ACollHandle coll, size_t device_id)
  {
    ...
    wait_recv<<<1, 32, 0, gpu_ctx.stream_>>>(ctx.d_send_cmd_[device_id], 
        ctx.d_ibv_atomic_recv_[device_id],
        num_procs_, my_proc_);
  }
  ...
  template void IbComm::post_send_command_a2a<__half>(HierA2ACollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <float>(HierA2ACollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <uint32_t>(HierA2ACollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <uint16_t>(HierA2ACollHandle coll, cudaStream_t dep_stream, size_t device_id);

  template void IbComm::post_send_command_a2a<__half>(HierA2AvCollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <float>(HierA2AvCollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <uint32_t>(HierA2AvCollHandle coll, cudaStream_t dep_stream, size_t device_id);
  template void IbComm::post_send_command_a2a <uint16_t>(HierA2AvCollHandle coll, cudaStream_t dep_stream, size_t device_id);
