...
namespace HugeCTR
{
  ...
  IbComm::ARCollContext::ARCollContext(IbComm* comm)
  {
    ...
    if (getenv("ONESHOT_NBLOCKS"))     { cfg_nblocks_ = atoi(getenv("ONESHOT_NBLOCKS")); }
    if (getenv("ONESHOT_ALIGN_BLOCK")) { cfg_align_block_ = atoi(getenv("ONESHOT_ALIGN_BLOCK")); }
    if (getenv("ONESHOT_MIN_BLOCK"))   { cfg_min_block_ = atoi(getenv("ONESHOT_MIN_BLOCK")); }
  ...
  void IbComm::ARCollContext::update_size(size_t ar_size)
  {
    ...
    PROXY_ASSERT_MSG((ar_size % (num_gpus_ * 16)) == 0, 
        "AR size needs to be aligned to num_gpus*16");
    
    ar_size_ = ar_size;
    blocksize_ = (cfg_nblocks_ - 1 + (cfg_align_block_ - 1 + ar_size) / cfg_align_block_) / cfg_nblocks_;
  ...
  template <> sharp_datatype IbComm::get_sharp_dtype<int>() { return SHARP_DTYPE_INT; }
  template <> sharp_datatype IbComm::get_sharp_dtype<uint32_t>() { return SHARP_DTYPE_UNSIGNED; }
  template <> sharp_datatype IbComm::get_sharp_dtype<__half>() { return SHARP_DTYPE_FLOAT_SHORT; }
  template <> sharp_datatype IbComm::get_sharp_dtype<float>() { return SHARP_DTYPE_FLOAT; }
  ...
  template void IbComm::set_ar_coll_buf<__half>(ARCollHandle coll, void* ar_ptr, const size_t ar_size, size_t device_id);
  template void IbComm::set_ar_coll_buf<float>(ARCollHandle coll, void* ar_ptr, const size_t ar_size, size_t device_id);
  template void IbComm::set_ar_coll_buf<uint32_t>(ARCollHandle coll, void* ar_ptr, const size_t ar_size, size_t device_id);
  ...
  all_reduce_cuda(void** __restrict__ d_peer_ptrs, const int numlines,
      size_t* d_coll_cmd_, size_t* h_rs_cmd_, size_t* d_ag_cmd_, size_t** flags,
      const int peerblocklines, const int numblocks, const int device_id)
  {
    ...
    if (threadIdx.x < RS_SYNC_THREADS)
    {
      ...
      while (blocklineoffset < numlines) {
        ...
        for (int line = blockIdx.x*AR_WORKER_THREADS + myThreadIdx;
            line < blocklines;
...

      {
        ...
        const int mydest = (device_id + 1 + ((threadIdx.x - basethread) >> 5) % (RANKS-1)) & (RANKS - 1);
        ...
        while (blocklineoffset < numlines) {
          ...
          if (threadIdx.x < maxthreadIdx) {
            ...
            const int aligned_elem = ((end_elem - start_elem) / 
                (myblockDim*gridDim.x*UNROLL)) * (myblockDim*gridDim.x*UNROLL);
            const int end_aligned = start_elem + aligned_elem;

            if(mythreadIdx==0) {
              while(*flag < gather_count) {}
              // printf("Gather flag received %llu %d %d %d %d %d %d %x\n", *flag, device_id, blockstart, blocklines, numlines, remainder, mydest, dest_ptr);
              gather_count++;
            }
    ...
    if ((threadIdx.x == 0) && (blockIdx.x == 0)) { *d_coll_cmd_ = (base_count + numblocks); }
  }
  ...
  void IbComm::all_reduce(ARCollHandle coll, cudaStream_t stream, size_t device_id) const
  {
    ...
    all_reduce_cuda<RANKS, T><<<ctx->cfg_nchannels_, warps*32, 0, stream>>>(
        ...
        gpu_ctx->h_rs_cmd_, gpu_ctx->d_ag_cmd_,
        gpu_ctx->d_flags_ptrs_.get_ptr(), 
        ctx->peer_blocklines_, ctx->num_blocks_, device_id_int);
  }
