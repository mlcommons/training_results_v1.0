...
template <typename commtype>
void AllToAll_Multi_NCCL<commtype>::communicate() {
  auto& stream = this->gpu_resource_->get_stream();
...
template <typename commtype>
void AllReduceComm<commtype>::communicate() {
  PROFILE_RECORD("allreduce.nccl_allreduce.start", stream, false);
  ar_comm_->all_reduce(ar_handle_, gpu_resource_->get_stream(), gpu_resource_->get_local_id());
  PROFILE_RECORD("allreduce.nccl_allreduce.stop", stream, false);
}
...
template <typename commtype>
void HierAll2Allv_Multi_IB<commtype>::update_sizes() {
  ib_comm_->pre_intra_update_a2a_coll_sizes(coll_handle_, send_sizes_, gpu_resource_->get_stream(),
                                            instance_id_);
}

template <typename commtype>
void HierAll2Allv_Multi_IB<commtype>::communicate() {
  ib_comm_->post_send_command_a2a<commtype>(coll_handle_, gpu_resource_->get_stream(),
                                            instance_id_);
  CK_CUDA_THROW_(cudaEventRecord(comm_event_, comm_stream_));
  CK_CUDA_THROW_(cudaStreamWaitEvent(gpu_resource_->get_stream(), comm_event_));
