...
namespace HugeCTR
{
  ...
  {
    __device__ __forceinline__ void sync_all_gpus_func(size_t** d_rem_barrier_flags,
  ...
  GPUBarrier::GPUBarrier(size_t num_gpus, const std::vector<int>& dev_list):
    num_gpus_(num_gpus),
    dev_list_(dev_list)
  {
  ...
  void GPUBarrier::sync_all_gpus_report_host_and_inc(size_t* d_report_count,
      size_t* h_report_ptr, const cudaStream_t stream, size_t device_id)
  {
    CK_CUDA_THROW_(cudaSetDevice(dev_list_[device_id]));
  ...
  {
    for (size_t g = 0; g < num_gpus_; g++) {
      cudaSetDevice(dev_list_[g]);
      cudaFree(d_rem_barrier_flags_[g]);
      cudaFree(d_barrier_flags_[g]);
      cudaFree(d_global_barrier_store_[g]);
    }
