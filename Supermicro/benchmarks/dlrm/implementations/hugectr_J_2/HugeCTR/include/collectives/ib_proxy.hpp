...
#define PROXY_ASSERT(expr) if (!(expr)) { ERROR_MESSAGE_(#expr); exit(1); }
#define PROXY_ASSERT_MSG(expr, msg) if (!(expr)) { ERROR_MESSAGE_(msg); exit(1); }
...
namespace HugeCTR
{
  ...
  struct M2PHierA2ACollInit
  {
    ...
    bool skip_barrier_;
    M2PHierA2ACollInit(HierA2ACollHandle handle_, CollSyncHelper* sync_helper,
        bool skip_barrier) : 
  ...
  struct M2PHierA2AvCollInit
  {
    ...
    CollSyncHelper* sync_helper_;
    bool skip_barrier_;
    M2PHierA2AvCollInit(HierA2AvCollHandle handle_, CollSyncHelper* sync_helper,
        bool skip_barrier) : 
      coll_handle_(handle_), sync_helper_(sync_helper), skip_barrier_(skip_barrier) {}
  };
  ...
  typedef boost::variant<boost::blank,
          ARBufInitCmd, ARCollInitCmd, ARUpdateSizeCmd,
          HierA2ABufInitCmd, HierA2ACollInitCmd,
          HierA2AvBufInitCmd, HierA2AvCollInitCmd,
          ProxyStateTransitionCmd> ProxyCmdType;
  ...
  struct ProxyCommand
  {
    public:
      ...
      void reset();
      void set_destroy() { destroy_ = 1; __sync_synchronize(); }
  ...
  struct IbvProxy
  {
    ...
    struct HierA2ACollContext
    {
      ...
      HierA2ACollContext(IbvProxy* proxy_ctx, HierA2AIbvContext* ibv_ctx,
    ...
    struct ARCollContext
    {
      ...
      size_t* d_gdr_ag_cmd_ = NULL;
      size_t* d_gdr_ag_storage_ = NULL;
