...
class InfrequentEmbedding {
 public:
  ...
  Tensor2<float *> embedding_vectors_pointers_;
  Tensor2<emtype *> gradients_pointers_;
  ...
  void hier_forward_network(const emtype *message_buffer, emtype *interaction_layer_input,
                            cudaStream_t stream);
  void forward_network_direct(emtype *interaction_layer_input, cudaStream_t stream);
  ...
  void fused_intra_update_network(const emtype *gradients, emtype **message_buffer,
                                  cudaStream_t stream);
  void update_model(const emtype *message_buffer, float lr, cudaStream_t stream);
  void hier_update_model(const emtype *message_buffer, float lr, cudaStream_t stream);
  void update_model_direct(float lr, cudaStream_t stream);
