...
### NVIDIA DGX2H (single node)

Launch configuration and system-specific hyperparameters for the NVIDIA DGX A100
single node submission are in the `config_DGX2.sh` script and in the `dgx2h.json` and `dgx2h.json`
config files.

Steps required to launch DLRM training with HugeCTR on a single NVIDIA DGX2H :
...
```
source config_DGX2.sh
CONT=<docker/registry>mlperf-nvidia:recommendation_hugectr LOGDIR=<path/to/output/dir> ./run_with_docker.sh
```
