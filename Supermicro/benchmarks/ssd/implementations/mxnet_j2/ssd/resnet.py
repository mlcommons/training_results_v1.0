...
class BasicBlockV1(HybridBlock):
    ...
    fuse_bn_relu: bool, default True
        Fuse batch normalization with relu activation.
    fuse_bn_add_relu: bool, default True
    ...
    fuse_conv_bn_add_relu : bool, default False
        Fuse convolution + bn + relu as part of batchnorm stats fprop fusion
    input_shape : list
        Shape of input tensor into residual block
        Required for batchnorm stats calculation when fuse_conv_bn_add_relu==True
        Incorrect values here WILL affect the batchnorm calculation
    output_shape : list
        Shape of input tensor into downsample convolution layer
        Required for batchnorm stats calculation when fuse_conv_bn_add_relu==True and downsample==True
        Incorrect values here WILL affect the batchnorm calculation
    start_index : int, default 0
        Index of the first FusedConv2DBNRelu block
        The following FusedConv2DBNAddRelu/FusedConv2DBNDualAddRelu will be start_index + 1
    norm_kwargs : dict
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0,
                 ...
                 fuse_conv_bn_add_relu=False, input_shape=[], output_shape=[], start_index=0,
                 norm_kwargs=None, layout='NHWC', **kwargs):
        super(BasicBlockV1, self).__init__(**kwargs)
        self.fuse_conv_bn_add_relu = fuse_conv_bn_add_relu
        if self.fuse_conv_bn_add_relu:
            assert len(input_shape) == len(layout)
        if downsample:
            assert len(output_shape) == len(layout)
            
        if not self.fuse_conv_bn_add_relu:
            # Network body
            ...
            # instance FusedConv2DBNRelu along with either
            # FusedConv2DBNAddRelu or 
            # FusedConv2DBNDualAddRelu
            # these fused kernels do not support use_se=True or fuse_bn_add_relu=False
            self.head = FusedConv2DBNRelu(
                prefix="convbn{}_".format(start_index),
            ...
            if not downsample:
                self.tail = FusedConv2DBNAddRelu(
                    prefix="convbn{}_".format(start_index+1),
                    input_shape=input_shape,
                    num_filter=channels,
                    kernel_size=3,
                    strides=1,
                ...
                self.tail = FusedConv2DBNDualAddRelu(
                    prefix="convbn{}_".format(start_index+1),
...
class ResNetMLPerf(HybridBlock):
    ...
    layout: str, default is 'NHWC'
        Dimension ordering of data and weight.
    """
    def __init__(self, batch_size, data_shape=300, classes=1000, thumbnail=False,
                 last_gamma=False, use_se=False,
                 norm_layer=GroupBatchNorm, fuse_bn_relu=True, fuse_bn_add_relu=True, bn_fp16=False, fuse_conv_bn_add_relu=False, norm_kwargs=None, layout='NHWC', **kwargs):
        super(ResNetMLPerf, self).__init__(**kwargs)
        with self.name_scope():
            block_type = BasicBlockV1
            input_shape = [batch_size, data_shape, data_shape, 4]
            ...
            self._stage_index = 1
            def calc_conv_output_dim(input_size, kernel_size, strides, padding):
                return math.floor((input_size + 2*padding-(kernel_size-1)-1)/strides) + 1
                
            conv_size = calc_conv_output_dim(300, 7, 2, 3)
            pool_size = calc_conv_output_dim(conv_size, 3, 2, 1)

            # stage1
            stage1_input_shape = [batch_size, pool_size, pool_size, 64]
            self.features.add(self._make_layer(block=block_type, layers=3, channels=64, stride=1, in_channels=64,
                                               ...
                                               input_shape=stage1_input_shape,
                                               output_shape=stage1_input_shape,
                                               norm_kwargs=norm_kwargs,
                                               layout=layout))
            # stage2
            stage2_conv_size = calc_conv_output_dim(pool_size, 3, 2, 1)
            stage2_output_shape = [batch_size, stage2_conv_size, stage2_conv_size, 128]
            self.features.add(self._make_layer(block=block_type, layers=4, channels=128, stride=2, in_channels=64,
                                               ...
                                               layout=layout))
            # stage3
            stage3_conv_size = calc_conv_output_dim(stage2_conv_size, 3, 1, 1)
            stage3_output_shape = [batch_size, stage3_conv_size, stage3_conv_size, 256]
            self.features.add(self._make_layer(block=block_type, layers=6, channels=256, stride=1, in_channels=128,
                                               ...
                                               bn_fp16=bn_fp16,
                                               fuse_conv_bn_add_relu=fuse_conv_bn_add_relu,
                                               input_shape=stage2_output_shape,
                                               output_shape=stage3_output_shape,
                                               norm_kwargs=norm_kwargs,
                                               layout=layout))

    def _make_layer(self, block, layers, channels, stride, in_channels=0,
                    last_gamma=False, use_se=False,
                    norm_layer=GroupBatchNorm, fuse_bn_relu=True, fuse_bn_add_relu=True, bn_fp16=False, fuse_conv_bn_add_relu=False, input_shape=[], output_shape=[],
                    norm_kwargs=None, layout='NHWC'):
