...
class SSDModel:
    FP16_BLACKLIST = ['gamma', 'beta', 'running_mean', 'running_var']  # List of params that can't be converted to FP16
    ...
    def init_model(self, force_reinit=False, ctx=None):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            def filter_name(name):
                return ("bias" in name) or ("weight" in name) \
                    or ("gamma" in name) or ("beta" in name)
            ...
            for param_name in self.net.collect_params():
                if filter_name(param_name) and force_reinit:
                    log_event(key=mlperf_constants.WEIGHTS_INITIALIZATION, 
                              metadata=dict(tensor=param_name))
    ...
    def train_val(self, train_iterator, start_epoch=1, end_epoch=80,
        ...
        log_event(key='d_batch_size', value=local_train_batch_size)
        epoch_size = train_iterator.epoch_size()
        iterations_per_epoch = math.ceil(epoch_size / global_train_batch_size)
        log_event(key=mlperf_constants.GRADIENT_ACCUMULATION_STEPS, value=1)
    ...
    def train_epoch(self, data_iterator, global_train_batch_size, iterations_per_epoch,
                ...
                if hvd.rank() == 0:
                    ...
                    log_event(key='tracked_stats',
                              value={'throughput': speed,
                                     'loss': loss0},
                              metadata={'step': (epoch, current_iter)})
        ...
        if log_interval and hvd.rank() == 0:
            speed = (i+1)*global_train_batch_size/epoch_time
            logging.info((f'[Training][Epoch {epoch}] '
                          f'training time: {epoch_time:.3f} [sec],'
                          f'avg speed: {speed:.3f} [imgs/sec],'
                          f'{name0}={loss0:.3f}'))
    ...
    def infer(self, data_iterator, log_interval=None):
        ...
        images_counter = 0
        tick = time.time()
        for i, (images, shapes, ids, padding) in enumerate(data_iterator):
            images_counter += images.shape[0]

            # run inference
            result, mask  = self.infer_net(images, self.anchors_mx, shapes, ids)
            ...
            # We apply the boolean_mask outside of inference network to allow static graph hybridization
            result = mx.nd.reshape(result, shape=(-1, 7))
            mask = mx.nd.reshape(mask, shape=(-1))
            result = mx.nd.contrib.boolean_mask(data=result, index=mask, axis=0)

            # append the results from all batches
            if result.shape[0] > 0:
