...
def get_training_iterator(pipeline, batch_size, synthetic=False):
    ...
    if batch_size != pipeline.max_batch_size:
        assert not pipeline.max_batch_size % batch_size, "batch size must divide the pipeline batch size"
        iterator = RateMatchInputIterator(ssd_iterator=iterator,
                                          input_batch_size=pipeline.max_batch_size,
...
class SSDDALITrainPipeline(dali.pipeline.Pipeline):
    ...
    def __init__(self, dataset_reader, num_workers, device_id, batch_size, data_shape,
        ...
        self.c_crop = dali.ops.RandomBBoxCrop(
            ...
            bbox_layout="xyXY",
            allow_no_crop=True,
            bytes_per_sample_hint=int(1488*1.1),  ## TODO: in DALI 1.0, set to [8,8,1488,372]
        ...
        if input_jpg_decode == 'gpu':
            ...
            self.m_decode = dali.ops.decoders.ImageSlice(device="mixed",
                    ...
                    bytes_per_sample_hint=int(727526*1.1))
            ...
            self.m_decode = dali.ops.decoders.Image(device='mixed', output_type=dali.types.RGB,
                                                    ...
                                                    cache_threshold=0,
                                                    hw_decoder_load=hw_decoder_load,
                                                    bytes_per_sample_hint=int(975596*1.1))
            self.g_slice = dali.ops.Slice(device="gpu", bytes_per_sample_hint=int(874804*1.1))
        ...
        self.g_resize = dali.ops.Resize(
            ...
            resize_x=data_shape,
            resize_y=data_shape,
            bytes_per_sample_hint=int(270000*1.1),
            temp_buffer_hint=int(640*300*32*3*4*1.1), #largest input dim * output dim * minibatch (default of 32) * num_channels * sizeof(float)
            min_filter=dali.types.DALIInterpType.INTERP_TRIANGULAR)

        self.g_hsv = dali.ops.Hsv(device="gpu", bytes_per_sample_hint=int(270000*1.1))
        self.g_brightness_contrast = dali.ops.BrightnessContrast(device="gpu", bytes_per_sample_hint=int(270000*1.1))
        ...
        self.g_normalize = dali.ops.CropMirrorNormalize(
            ...
            bytes_per_sample_hint=int(720000*1.1))
        ...
        self.g_box_encoder = dali.ops.BoxEncoder(
            ...
            scale=data_shape,
            bytes_per_sample_hint=int(139712*1.1))  ## TODO: in DALI 1.0, set to [139712,34928]

        self.g_cast = dali.ops.Cast(device="gpu", dtype=dali.types.FLOAT, bytes_per_sample_hint=int(34928*1.1))
