...
# A ScaleBiasAddRelu wrapper that responds to the input layout
def scale_bias_add_relu(data, io_layout, batchnorm_layout,addend, x_equiv_scale, x_equiv_bias, z_equiv_scale, z_equiv_bias,
        x_gamma, x_beta, x_mean, x_invvar, z_gamma, z_beta, z_mean, z_invvar, dual_scale_bias, fused_add, name):
    transposed_data_as_needed = transform_layout(data, io_layout, batchnorm_layout)
    if dual_scale_bias or fused_add:
    ...
    if dual_scale_bias:
        batchnormed, _ = mx.sym.ScaleBiasAddRelu(dataX = data, dataZ = addend,
                  ...
                  x_gamma = x_gamma, x_beta = x_beta, x_mean = x_mean,
                  x_invvar = x_invvar, z_gamma = z_gamma, z_beta = z_beta,
        ...
        if fused_add:
            batchnormed, _ = mx.sym.ScaleBiasAddRelu(dataX = data, dataZ = addend,
                      x_equiv_scale = x_equiv_scale , x_equiv_bias = x_equiv_bias,
                      x_gamma = x_gamma, x_beta = x_beta, x_mean = x_mean,
                      x_invvar = x_invvar, act_type='relu', layout = 'NHWC',
                      dual_scale_bias = False, fused_add = True,
                      name = name)
        else:
            batchnormed, _ = mx.sym.ScaleBiasAddRelu(dataX = data,
                      x_equiv_scale = x_equiv_scale , x_equiv_bias = x_equiv_bias,
                      x_gamma = x_gamma, x_beta = x_beta, x_mean = x_mean,
                      x_invvar = x_invvar, layout = 'NHWC',
                      act_type='relu', dual_scale_bias = False, fused_add = False,
                      name = name)
    # Transpose back to i/o layout as needed
    return transform_layout(batchnormed, batchnorm_layout, io_layout)
...
def conv_nchw_out_shape(nchw_inshape, num_filter, kernel, stride=(1,1), pad=(0,0)):
    ...
    return (n, num_filter, out_shape_h, out_shape_w)
...
def residual_unit_norm_conv(rank, data, nchw_inshape, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    if bottle_neck:
        ...
        bn1 = scale_bias_add_relu(conv1, conv_layout, batchnorm_layout, None,
                bn1_equiv_scale, bn1_equiv_bias, None, None,
                bn1_gamma_out, bn1_beta_out, bn1_saved_mean, bn1_saved_inv_std, None, None,
                None, None, False, False, name+"_bnapply1")
        ...
        # Second NormalizedConvolution: [no Stats Apply] [no Relu] Convolution Stats-Gen
        (conv2, conv2_sum, conv2_sum_squares) = \
            mx.sym.NormalizedConvolution(bn1, no_equiv_scale_bias=True, act_type=None,
        ...
        bn2 = scale_bias_add_relu(conv2, conv_layout, batchnorm_layout, None, bn2_equiv_scale, bn2_equiv_bias, None, None,
                    bn2_gamma_out, bn2_beta_out, bn2_saved_mean, bn2_saved_inv_std, None, None,
                    None, None, False, False, name+"_bnapply2")
        shape = resnet_batchnorm_log(shape, momentum=bn_mom, eps=bn_eps, center=True, scale=True, training=True)
        shape = resnet_relu_log(shape)

        # Third NormalizedConvolution: Convolution Stats-Gen
        (conv3, conv3_sum, conv3_sum_squares) = \
            mx.sym.NormalizedConvolution(bn2, no_equiv_scale_bias=True, act_type=None,
        ...
        if dim_match:
            ...
            dbar = True
            shortcut, conv1sc_sum, conv1sc_sum_squares = mx.sym.NormalizedConvolution(data, no_equiv_scale_bias=True, act_type=None,
                                     num_filter=num_filter, kernel=(1,1), stride=stride, pad=(0,0),
                                     name=name + '_conv1sc', layout=conv_layout)
            proj_shape = resnet_conv2d_log(nchw_inshape[1:], stride, int(num_filter), False)
            elem_count = element_count((batch_size,) + proj_shape)
        ...
        if fuse_bn_add_relu:
            if dbar:
                ...
                return (scale_bias_add_relu(conv3, conv_layout, batchnorm_layout, shortcut, bn3_equiv_scale, bn3_equiv_bias, bn1sc_equiv_scale, bn1sc_equiv_bias,
                    bn3_gamma_out, bn3_beta_out, bn3_saved_mean, bn3_saved_inv_std, bn1sc_gamma_out, bn1sc_beta_out,
                    bn1sc_saved_mean, bn1sc_saved_inv_std, True, True,name+"_dbasr3"),nchw_shape)
                ...
                shape = resnet_relu_log(shape)
                nchw_shape = (batch_size, ) + shape
                return (scale_bias_add_relu(conv3, conv_layout, batchnorm_layout, shortcut, bn3_equiv_scale, bn3_equiv_bias, None, None,
                    bn3_gamma_out, bn3_beta_out, bn3_saved_mean, bn3_saved_inv_std, None, None,
                    None, None, False, True, name+"_sbar3"),nchw_shape)
