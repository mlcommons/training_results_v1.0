...
# start syncing selfplay games written by current node with node 0 
echo ${root_node} 
./ml_perf/scripts/start_sync_selfplay.sh --base_dir=${base_dir} --root_node=${root_node} 
...
for ((device=0;device<PHY_CORES;device++)); do
  ...
  ml_perf/scripts/loop.sh numactl --physcpubind=${device} ./bazel-bin/cc/concurrent_selfplay \
    ...
    --output_dir="${selfplay_local_dir}/\$MODEL/${device}" \
