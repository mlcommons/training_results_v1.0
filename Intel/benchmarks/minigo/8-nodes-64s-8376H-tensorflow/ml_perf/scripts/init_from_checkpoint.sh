...
then
  ...
  echo Create local selfplay dir for each node
  mpirun -bootstrap ssh -ppn 1 -hosts $HOSTLIST mkdir -p $selfplay_local_dir
  mpirun -bootstrap ssh -ppn 1 -hosts $HOSTLIST rm -rf $selfplay_local_dir/*  
...
# copy selfplay files to selfplay master dir on node 0
echo "Copy selfplay files to node 0 storage"
mkdir -p ${selfplay_master_dir}
rm -rf ${selfplay_master_dir}/*
rsync -r --append ${selfplay_dir} `dirname ${selfplay_master_dir}`
ls ${selfplay_master_dir}
