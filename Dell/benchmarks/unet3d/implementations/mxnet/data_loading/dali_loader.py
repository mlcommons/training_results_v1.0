...
class ExternalInputIterator(object):
    def __init__(self, batch_size, device_id, shard_id, num_shards, filelist, seed, in_gpu=False, stick_to_shard=False):
        self.batch_size = batch_size
        self.filelist = filelist
        self.data_set_len = len(self.filelist)
        self.shard_size = m.ceil(self.data_set_len / num_shards)
        if stick_to_shard:
            num_fixed_shards = self.data_set_len % num_shards
            if not num_fixed_shards:
                num_fixed_shards = num_shards
            self.num_fixed_shards = num_fixed_shards

            if shard_id < num_fixed_shards:
                start = shard_id * self.shard_size
                my_shards = [i for i in range(start, start + self.shard_size)]
            else:
                start = (num_fixed_shards * self.shard_size) + ((shard_id - num_fixed_shards) * (self.shard_size - 1))
                my_shards = [i for i in range(start, start + self.shard_size - 1)]
                my_shards.append(shard_id % (num_fixed_shards * self.shard_size))
            self.filelist = [filelist[i] for i in my_shards]
        ...
        self.data = {}
        self.start_from_idx = (self.shard_id * self.shard_size) % self.n
        self.order_state = [i for i in range(len(self.filelist))]
        self.order = self.order_state[:]
        self.in_gpu = in_gpu
        self.stick_to_shard = stick_to_shard
        self.num_shards = num_shards

    def __iter__(self):
        if self.stick_to_shard:
            self.rng.shuffle(self.order_state)
            self.order = self.order_state[:]
            self.i = 0
        else:
    ...
    def __next__(self):
        if self.i >= self.start_from_idx + self.shard_size:
            if self.stick_to_shard:
                self.rng.shuffle(self.order_state)
                self.order = self.order_state[:]
                self.i = 0
                num_fixed_shards = self.num_fixed_shards
                if self.shard_id >= num_fixed_shards:
                    p = self.rng.integers(0, self.data_set_len - 1)
                    if p < self.num_shards - num_fixed_shards:
                        self.order = [0 if x == self.shard_size - 1 else x for x in self.order_state]
                    elif p < 2 * (self.num_shards - num_fixed_shards):
                        self.order = [1 if x == self.shard_size - 1 else x for x in self.order_state]
            else:
                self.i = self.start_from_idx
                self.rng.shuffle(self.order)

        if self.order[self.i] not in self.data:
            image = np.load(self.filelist[self.order[self.i]])
            if self.in_gpu:
                with cp.cuda.Device(self.device_id):
                    image = cp.asarray(image)
            self.data[self.order[self.i]] = image

        image = self.data[self.order[self.i]]
        self.i += 1
...
class BasicPipeline(Pipeline):
    ...
    def biased_crop_fn(self, img, label, use_cached_loader):
        ...
        # img, label = fn.slice([img, label], anchor, self.crop_shape, axes=[1, 2, 3],
