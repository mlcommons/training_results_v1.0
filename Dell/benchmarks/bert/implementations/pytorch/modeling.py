...
class LinearDropoutAdd(torch.nn.Linear):
    ...
    def forward(self, input, residual):
        ...
        if self.bias is None:
            ...
            result = jit_bias_dropout_add(linear_out, self.bias.expand_as(residual), residual, self.p, is_training=self.training)
        return result
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        if not config.fused_dropout_add:
            ...
            self.dense = LinearDropoutAdd(config.intermediate_size, config.hidden_size, bias=True, p=config.hidden_dropout_prob)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.p = config.hidden_dropout_prob
        self.fused_dropout_add = config.fused_dropout_add
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, masked_lm_labels):
        if self.dense_seq_output:
            ...
            sequence_flattened = torch.index_select(sequence_output.view(-1,sequence_output.shape[-1]), 0, torch.nonzero(masked_lm_labels.view(-1) != 0, as_tuple=False).squeeze())
