...
```
    ...
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=${DGXSYSTEM} bash run_with_docker.sh
...
```
    ...
    CONT="mlperf-nvidia:minigo" DATADIR=<path/to/store/checkpoint> LOGDIR=<path/to/output/dir> DGXSYSTEM=${DGXSYSTEM} sbatch -N $DGXNNODES -t $WALLTIME run.sub
```
