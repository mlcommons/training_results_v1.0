...
def get_tf_mapping(config, task="PRETRAINING"):
    ...
    optimizer_states = {}
    for tf_var, onnx_var in tf_to_onnx.items():
        optimizer_states[tf_var+"/LAMB"] = popart.reservedAccl1Prefix() + onnx_var
        optimizer_states[tf_var+"/LAMB_1"] = popart.reservedAccl2Prefix() + onnx_var
    tf_to_onnx.update(**optimizer_states)
...
def generate_initializers(config, map_names, load_data, mapping, transform={}):
    ...
    for name, array in zip(map_names, load_data):
        ...
        if config.lamb_m_dtype == "FLOAT16":
            lamb_m_dtype = np.float16

        if popart.reservedAccl1Prefix() in mapping[name]:
            if array.dtype != lamb_m_dtype:
                array = array.astype(lamb_m_dtype)
        elif popart.reservedAccl2Prefix() in mapping[name]:
            if array.dtype != np.float32:
                array = array.astype(np.float32)
        elif array.dtype != config.dtype:
        ...
        if "Embedding/Embedding_Dict" in mapping[name]:
            ...
            if diff > 0:
                ...
                logger.debug(f"Cropping the vocabulary may negatively effect performance. From {tf_vocab_length} to {config.vocab_length}")
        ...
        if "MLM/ProjectionB" in mapping[name]:
            ...
            if diff > 0:
                ...
                logger.warning(f"Cropping the projection bias from {tf_vocab_length} to {config.vocab_length}")
                array = np.array(array[:config.vocab_length])

        if "Embedding/Positional_Dict" in mapping[name]:
            max_pos, hidden_len = array.shape
            if max_pos > config.max_positional_length:
                array = array[:config.max_positional_length, :]

            # Otherwise just copy the positional embeddings over and over again as is done in longformer
            elif max_pos < config.max_positional_length:
                logger.warning(f"Not enough positional embeddings in checkpoint, copying to match length...")
                array = array[np.mod(np.arange(config.max_positional_length), max_pos)]
