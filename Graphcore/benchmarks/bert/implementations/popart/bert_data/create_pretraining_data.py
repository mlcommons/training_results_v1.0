...
def write_instance_to_example_files(instances, tokenizer, max_seq_length,
                                    max_predictions_per_seq, output_files, max_samples=-1):
  ...
  for (inst_index, instance) in enumerate(tqdm(instances)):
    ...
    next_sentence_labels = [1] if instance.is_random_next else [0]
    ...
    features["masked_lm_weights"] = list(map(int, masked_lm_weights)),
    ...
    if inst_index < 20:
      ...
      for feature_name in features.keys():
        print("%s: %s" % (feature_name, " ".join([str(x) for x in features[feature_name]])))
...
def create_training_instances(input_files, tokenizer, max_seq_length,
  ...
  print(f"Tokenizing {num_lines} lines int {len(all_documents)} docs took {time.time() - start:3.2f} seconds.")
      ...
      if current_chunk:
        ...
        (tokens, masked_lm_positions,
         masked_lm_labels) = create_masked_lm_predictions(
             tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng, max_seq_length)
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  # CHANGE: Make sure that the desired number of max_predictions_per_seq is used
  # to prevent issues with too many tokens needed. Ie
  # sequence_length 384, max_predictions_per_seq 60. if num_to_predict is 58
  ...
  num_to_predict = max(num_to_predict, len(tokens) - max_seq_length + max_predictions_per_seq)
...
def main(args):
  ...
  for input_pattern in args.input_file.split(","):
    input_files_glob.extend(glob.glob(input_pattern))

  num_files = len(input_files_glob)
  print(f"*** Reading {num_files} input files in batches of 10***")
  for i in tqdm(range(0, num_files, 10)):
    input_files = input_files_glob[i:min(i + 10, num_files)]
    for input_file in input_files:
    ...
    output_files = [args.output_file + f"_{i//10}"]
    print("*** Writing to output files ***")

    write_instance_to_example_files(instances, tokenizer, args.sequence_length,
                                    args.max_predictions_per_seq, output_files, args.max_samples)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--do-whole-word-mask", type=int, default=False)
