...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.end_position = end_position
        self.is_impossible = is_impossible
        # Added for PopART implementation
        self.padding_start_index = padding_start_index
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        #     logger.debug('Converting %s/%s pos %s neg %s', example_index, len(examples), cnt_pos, cnt_neg)
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            # Index at which padding starts.
            padding_start_index = len(input_ids)
            ...
            # MAJOR CHANGE: Remove impossible features.
            # Reverting this change. It is under investigation. T12851
            # if not (is_training and span_is_impossible):
            features.append(
                InputFeatures(
                    unique_id=unique_id,
                    ...
                    padding_start_index=padding_start_index))
            unique_id += 1
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    if len(orig_ns_text) != len(tok_ns_text):
        if verbose_logging:
            logger.debug("Length not equal after stripping spaces: '%s' vs '%s'",
