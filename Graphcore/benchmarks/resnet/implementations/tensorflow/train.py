...
from optimisers import IPU_Momentum, IPU_LARS
...
def get_optimizer(opts):
    ...
    elif opts['optimiser'] == 'IPU_momentum':
        opt_fun = partial(moving_avg_update(IPU_Momentum),
                          momentum=opts['momentum'],
                          momentum_dtype=tf.float16 if opts["fp16_momentum"] else None,
                          gradient_dtype=tf.float16 if opts["fp16_gradients"] else None)
    ...
    elif opts['optimiser'] == 'IPU_LARS':
        opts['lars_skip_list'] += ['batch_norm/moving_']
        opt_fun = partial(moving_avg_update(IPU_LARS),
                          momentum=opts['momentum'],
                          weight_decay=opts['lars_weight_decay'],
                          eta=opts['lars_eeta'],
                          epsilon=opts['lars_epsilon'],
                          exclude_filter=opts['lars_skip_list'],
                          momentum_dtype=tf.float16 if opts["fp16_momentum"] else None,
                          gradient_dtype=tf.float16 if opts["fp16_gradients"] else None
                          )
    else:
...
def basic_training_step(data_dict, model, opts, learning_rate):
    ...
    if opts["eight_bit_io"]:
        ...
        data_dict['image'] = tf.cast(data_dict['image'], dtype=tf.float16 if dtypes[0] == '16' else tf.float32)
...
def basic_pipelined_training_step(model, opts, learning_rate, infeed, outfeed, iterations_per_step=1):
    ...
    def first_stage(learning_rate, data_dict, pipeline_stage=None, idx=None):
        ...
        if opts['dataset'] == 'imagenet' and not opts.get('hostside_norm') and idx == 0:
    ...
    return pipelining_ops.pipeline(computational_stages=computational_stages,
                                   ...
                                   gradient_accumulation_dtype=tf.float16 if opts["fp16_gradients"] else None,
                                   name="Pipeline")
...
def training_graph(model, opts, iterations_per_step=1):
    ...
    with train_graph.as_default(), ExitStack() as stack:
        ...
        if strategy:
            ...
            outfeed = [strategy.reduce(tf.distribute.ReduceOp.MEAN, v)/opts["gradient_accumulation_count"] for v in
                       outfeed]
        else:
            outfeed = [v/opts["gradient_accumulation_count"] for v in outfeed]
...
def train_process(model, LR_Class, opts):
    ...
    if opts['optimiser'] == 'momentum':
        warmup_iterations = iterations * opts["warmup_epochs"] // opts["epochs"]
    ...
    while i < iterations:
        ...
        if opts['epochs_per_ckpt'] and ckpt_this_step and round(epoch) == opts['epochs'] and (i + iterations_per_step) < iterations:
            ckpt_this_step = False

        if math.floor(epoch) == log_epoch and int(round(epoch)) != int(opts['epochs']):
        ...
        if sync_this_step and opts["use_popdist"]:
...
def add_training_arguments(parser):
    ...
    tr_group.add_argument('--optimiser', type=str, default="SGD",
                          choices=['SGD', 'RMSprop', 'momentum', 'IPU_momentum', 'LARS', 'IPU_LARS'],
                          help="Optimiser")
    ...
    tr_group.add_argument('--fp16-gradients', action="store_true")
    tr_group.add_argument('--fp16-momentum', action="store_true")
    ...
    tr_group.add_argument('--min-remote-tensor-size', type=int, default=0,
                          help='The minimum remote tensor size for partial variable offloading')
...
if __name__ == '__main__':
    ...
    opts = configurations.parse_config(opts, parser, known_args_only=True)
    opts = vars(opts)
    print(opts)
