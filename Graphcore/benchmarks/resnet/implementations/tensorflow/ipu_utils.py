...
def get_config(prng=False,
    ...
    config = utils.set_optimization_options(config,
                                            minimum_remote_tensor_size=min_remote_tensor_size, # 17MB offloaded from 49MB of momentum tensor (useful if variable-offloading is set to True)
    ...
    if "GCL_REAL_COLLECTIVES" in os.environ:
       ...
       config = utils.set_gcl_options(config, gcl_options={"useGclCollectives": "true", })
       config = utils.set_io_tile_options(config, num_io_tiles=iotiles)
    ...
    config = utils.set_compilation_options(config, {
        ...
        "prng.enableStochasticRounding": "true" if prng else "false",
