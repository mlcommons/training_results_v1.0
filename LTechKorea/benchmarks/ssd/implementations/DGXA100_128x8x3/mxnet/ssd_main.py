...
VALID_MODES = ['train']  # TODO(ahmadki): add inference
...
def parse_args():
    ...
    parser.add_argument('--cocoapi-threads', type=int, default=6,
...
def main(async_executor=None):
    ...
    if args.async_val:
        ...
        # 'warmup' the async executor by forking a dummy function before real training starts
        if hvd.rank() == 0:
            async_executor.submit(tag="dummy_fn", fn=lambda: 1)
    else:
    ...
    # First dry run is to initialize the network parameters.
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        tmp_fname = os.path.join(tmp_dir, str(hvd.rank()))
        # Now that parameters are initialized, we can save a copy of the trainer states (momentum).
        model.trainer.save_states(tmp_fname)
        # Second dry run is to initialize gradients
        for images, box_targets, cls_targets in dummy_train_iterator:
            model.train_step(images=images, box_targets=box_targets, cls_targets=cls_targets)
            model.trainer.set_learning_rate(0.0)
            model.trainer.step(1)
        # Restore the original (empty) states
        model.trainer.load_states(tmp_fname)
    mx.ndarray.waitall()
    logging.info('Done')
    ...
    logging.info('Cleaning the dummy pipelines ...')
    ...
    # re-initialize the model and zero gradients as the dry runs have changed the weights
