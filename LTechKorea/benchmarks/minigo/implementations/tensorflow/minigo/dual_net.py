...
def _get_nontpu_estimator():
    ...
    run_config = tf.estimator.RunConfig(
        save_checkpoints_steps=max(1000, FLAGS.iterations_per_loop) if hvd.rank() == 0 else None,
...
def freeze_graph(model_path, use_trt=False, trt_max_batch_size=8,
    ...
    minigo_model.write_graph_def(out_graph, metadata, model_path + '.evalfp32minigo')
    ...
    minigo_model.write_graph_def(out_graph, metadata, model_path + '.stagedmodel')
