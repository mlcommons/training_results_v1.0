...
class FusedSGD(Optimizer):
    ...
    def step(self, dict_fp16, dict_fp32, closure=None, scale=1.):
        ...
        for i, (group, fp32_p, fp32_g, fp16_p, fp16_g, fp32_from_fp16_p) in enumerate(zip(self.param_groups, 
            ...
            if self.debug_prints and torch.distributed.get_rank() == 0:
                def cam(tensors):
                    with torch.no_grad():
                        abs_max = torch.zeros([1], device='cuda')
                        for t in tensors:
                            curr_abs_max = torch.max(torch.abs(t.float()))
                            abs_max = torch.max(abs_max, curr_abs_max)
                        return abs_max

                print("group %d :: lr=%.2e, [fp32_g=%.2e, fp32_p=%.2e, fp32_m=%.2e], [fp16_g=%.2e, fp32_from_fp16_p=%.2e, fp32_from_fp16_m=%.2e, fp16_p=%.2e]" % 
                        (i, lr, cam(fp32_g), cam(fp32_p), cam(fp32_m), cam(fp16_g), cam(fp32_from_fp16_p), cam(fp32_from_fp16_m), cam(fp16_p)))
