...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  if [ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]; then
    ...
    if [[ "${DGXSYSTEM}" == MF5488* ]]; then
        CMD=( './bind.sh' '--cpu=nf5488_topology.sh' '--mem=nf5488_topology.sh' '--' 'python' '-u' )
    else
  ...
  if [[ "${DGXSYSTEM}" == NF5488M6 ]]; then
    CMD=( 'python' '-u' '-m' 'bind_launch_nf5488m6' "--nsockets_per_node=${DGXNSOCKET}" \
