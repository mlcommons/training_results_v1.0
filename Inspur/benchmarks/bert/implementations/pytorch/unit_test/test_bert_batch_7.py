...
#@parameterized_class(('dense_seq_output'), [(True),(False)])
#@parameterized_class([{'param_dense_seq_output': True}, {'param_dense_seq_output': False}])
class TestBert(unittest.TestCase):
  @classmethod
  def setUpClass(self):
    self.device = 'cuda:0'
    self.data_path = test_data_path.get_path() + '/gold/batch_7/'
    ...
    #self.bert_config.dense_seq_output = self.param_dense_seq_output
    #self.bert_config.dense_seq_output = False #True
    self.bert_config.dense_seq_output = False
  ...
  def test_bert_encoder_self_attention_score(self, layer_idx=0):
    ...
    if layer_idx == 6:
        tolerance = 2.25e-4
    test_pass = bins_absolute[-1] < tolerance
  ...
  def test_bert_total_loss(self):
    ...
    masked_lm_ids = self.tf_masked_lm_ids.long()
    next_sentence_label = self.tf_next_sentence_label.long()

    batch_size = input_ids.shape[0]
    masked_lm_labels = torch.zeros((batch_size, self.max_seq_len), dtype=torch.long).to(self.device)
    offsets = torch.arange(0, batch_size).to(self.device).view(-1, 1) * self.max_seq_len
    masked_lm_positions_offset = masked_lm_positions + offsets
    masked_lm_labels.view(-1)[masked_lm_positions_offset[masked_lm_positions_offset != 0]] = masked_lm_ids.view(-1)[masked_lm_positions.view(-1) != 0]
  ...
  def test_bert_model_sequence_output(self):
    ...
    tolerance = 1.8e-4
    test_pass = bins_absolute[-1] < tolerance  # A100 exception
  ...
  def test_bert_mlm_prediction_scores(self):
    ...
    masked_lm_positions = masked_lm_positions.view(-1, self.max_pred_per_seq)
    masked_lm_labels = masked_lm_labels.view(-1, self.max_seq_len)
    masked_lm_labels = masked_lm_labels.reshape(-1, self.max_seq_len)
    ...
    if not self.bert_config.dense_seq_output:
      pyt_result = pyt_result.view(-1, pyt_result.shape[-1])
      pyt_result = torch.index_select(pyt_result, 0, masked_lm_labels.view(-1).nonzero().squeeze(1))
    
    pyt_result = pyt_result.detach().cpu().numpy()

    tf_result = gv.tf_tensors['cls/predictions/mlm_logits_bias']
    tf_result = nn.functional.pad(torch.from_numpy(tf_result), (0,6), "constant", 0)
    tf_result = torch.index_select(tf_result, 0, masked_lm_positions.view(-1).nonzero().squeeze(1).detach().cpu())
