...
class BertNetworkWithLoss(nn.Cell):
    ...
    def __init__(self, config, is_training, bert=None, use_one_hot_embeddings=False):
...
class BertPretrainEval(nn.Cell):
    """
    Evaluation MLM prediction scores.
    """
    def __init__(self, config, bert=None):
        super(BertPretrainEval, self).__init__()
        if bert is None:
            self.bert = BertPreTraining(config, False, False)
        else:
            self.bert = bert
    ...
    def construct(self, input_ids, input_mask, token_type_id, nsp_label, masked_pos, masked_ids, masked_weights):
        """Calculate prediction score"""
        bs, _ = F.shape(input_ids)
        mlm, nsp = self.bert(input_ids, input_mask, token_type_id, masked_pos)
        mlm_index = self.argmax(mlm)
        mlm_index = F.reshape(mlm_index, (bs, -1))
        mlm_acc = F.equal(mlm_index, masked_ids)
        mlm_acc = F.cast(mlm_acc, mstype.float32)
        mlm_acc = mlm_acc * masked_weights
        mlm_acc = self.sum(mlm_acc)
        mlm_total = self.sum(masked_weights)

        if self.reduce_flag:
            mlm_acc = self.allreduce(mlm_acc)
            mlm_total = self.allreduce(mlm_total)

        return mlm_acc, mlm_total
