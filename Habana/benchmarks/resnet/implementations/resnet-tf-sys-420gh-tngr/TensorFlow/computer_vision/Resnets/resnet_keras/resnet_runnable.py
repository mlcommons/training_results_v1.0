...
class ResnetRunnable(standard_runnable.StandardTrainable,
  ...
  def eval_end(self):
    ...
    if self.dist_eval:
      self.test_accuracy.total.assign(hvd.allreduce(self.test_accuracy.total, op=hvd.Sum))
      self.test_accuracy.count.assign(hvd.allreduce(self.test_accuracy.count, op=hvd.Sum))
    ...
    if self.flags_obj.target_accuracy is not None:
      past_treshold = eval_accuracy >= self.flags_obj.target_accuracy
      if horovod_enabled():
        past_treshold = hvd.allreduce(tf.cast(past_treshold, tf.float32),
                                      op=hvd.Sum) > 0

    continue_training = True
    if (past_treshold or eval_accuracy <= 0.002):
