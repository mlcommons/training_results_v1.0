...
from TensorFlow.common.habana_layer_norm import HabanaLayerNormalization
...
def layer_norm(input_tensor):
  ...
  if input_tensor.dtype == tf.float16:
    try:
      from utils.fused_layer_norm import fused_layer_norm
      return fused_layer_norm(
          inputs=input_tensor, begin_norm_axis=-1, begin_params_axis=-1, scope='LayerNorm',
          use_fused_batch_norm=True)
    except ImportError:
      return HabanaLayerNormalization(axis=-1, name='LayerNorm', epsilon=1e-12)(input_tensor)
  else:
    return HabanaLayerNormalization(axis=-1, name='LayerNorm', epsilon=1e-12)(input_tensor)
